...
class HindolaKirtan extends StatefulWidget {
  const HindolaKirtan({Key? key}) : super(key: key);

  @override
  State<HindolaKirtan> createState() => _HindolaKirtanState();
}

class _HindolaKirtanState extends State<HindolaKirtan> {
  ...
  void fetchUrl() async {
    try {
      var response = await http.get(Uri.parse(AppUrl.hindodaKirtanApi),headers: {
  ...
  Widget build(BuildContext context) {
    ...
    return statusUrl.isNotEmpty ? Scaffold(
        ...
        appBar: AppBar(
          elevation: 0.0,
          title: const Text(hindodaKirtan),
          actions: [
            IconButton(
                onPressed: () {
